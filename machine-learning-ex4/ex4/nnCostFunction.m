...
% NOTES
% m - num of examples in training set
% K - num of classes in output
% L - number of layers in network 
% i - index of current example
% s - number of units in layer

y_matrix = eye(num_labels); % 10 x 10
...
for l=1 : 2
    for i=1 : s_array(l)
        for j=1 : s_array(l+1)
            theta = ifelse(l == 1, Theta1(:, 2:end), Theta2(:, 2:end));
            reg_term = reg_term + (theta(j, i))**2;
        endfor
    endfor
endfor

J = (1 / m) * cost + (lambda / (2 * m)) * reg_term;
